      ...
      try {
        ...
        try {
          if (insertedPaper?.id) {
            ...
            if (queueError) {
              console.warn(`添加翻译任务失败: ${queueError.message}`)
            } else {
              console.log(`已添加翻译任务到队列: ${doi}`)
            }
            
            // 添加PDF下载任务到队列（用于全文分析）
            const { error: pdfQueueError } = await supabase
            ...
            if (pdfQueueError) {
              console.warn(`添加PDF下载任务失败: ${pdfQueueError.message}`)
            } else {
              console.log(`已添加PDF下载任务到队列: ${doi}`)
            }
          } else {
             console.warn(`无法获取插入的论文ID，跳过翻译任务: ${doi}`)
          }
        } catch (queueError) {
          console.warn(`添加任务到队列异常:`, queueError)
        }
